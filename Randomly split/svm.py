...
super_para1 = [20,1,0.1]
super_para2 = [30,1,0.1]
super_para3 = [30,1,0.1]
super_para4 = [30,1,0.1]
super_para5 = [30,1,0.1]
super_para6 = [30,1,0.1]
...
for j in range(len(name_list)):
    ...
    for i in range(1, 1000):
        ...
        model_svr = SVR(kernel='rbf', C=super_para[j][0], epsilon=super_para[j][2], gamma=super_para[j][1])

        model_svr.fit(data_train_x_nor, data_train_y_nor)

        y_pred_test_nor = model_svr.predict(data_test_x_nor)
        y_pred_train_nor = model_svr.predict(data_train_x_nor)
    ...
    results_df.to_csv(f'data_evaluate\\svm_{path_list[j]}_evaluate.csv', index=False)

    my_list.append(f"{path_list[j]} 最佳R²值: {best_r2} 对应的随机种子是: {best_seed}")

with open("svm.txt", "w", encoding="utf-8") as file:
    for item in my_list:
        file.write(item + "\n")