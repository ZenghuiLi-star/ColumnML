...
for j in range(len(name_list)):
    ...
    for i in range(1, 1000):
        ...
        model_rf = RandomForestRegressor(
            n_estimators=super_para[j][0],
            min_samples_leaf=super_para[j][1],
            min_samples_split=super_para[j][2],
            n_jobs=-1,
        )
    ...
    results_df.to_csv(f'data_evaluate\\rf_{path_list[j]}_evaluate.csv', index=False)

    my_list.append(f'{path_list[j]} 最佳R²值: {best_r2} 对应的随机种子是: {best_seed}')


with open('rf.txt', 'w', encoding='utf-8') as file:
    for item in my_list:
        file.write(item + '\n')
