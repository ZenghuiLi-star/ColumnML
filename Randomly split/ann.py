...
for j in range(len(name_list)):
    ...
    for i in range(1, 1000):
        ...
        model = tf.keras.Sequential([
            tf.keras.layers.Dense(128, activation='relu', input_dim=data_train_x.shape[1]),
        ...
        model.fit(data_train_x_nor, data_train_y_nor, epochs=100, batch_size=32, verbose=1)
    ...
    results_df.to_csv(f'data_evaluate\\ann_{path_list[j]}_evaluate.csv', index=False)

    my_list.append(f"{path_list[j]} 最佳R²值: {best_r2} 对应的随机种子是: {best_seed}")

with open("ann.txt", "w", encoding="utf-8") as file:
    for item in my_list:
        file.write(item + "\n")