...
super_para1 = [100,10,0.2,2]
super_para2 = [100,10,0.2,2]
super_para3 = [100,9,0.1,5]
super_para4 = [300,10,0.1,5]
super_para5 = [100,10,0.1,1]
super_para6 = [500,10,0.2,5]
...
for j in range(len(name_list)):
    ...
    for i in range(1, 1000):
        ...
        model_xgb = xgb.XGBRegressor(n_estimators=super_para[j][0],max_depth=super_para[j][1],learning_rate=super_para[j][2],reg_lambda=super_para[j][3],n_jobs=128)


        model_xgb.fit(data_train_x, data_train_y)


        y_pred_test = model_xgb.predict(data_test_x)
        y_pred_train = model_xgb.predict(data_train_x)
        ...
        _, _, _, _, r2_train = evaluate_regress(data_train_y, y_pred_train1)
    ...
    results_df = pd.DataFrame(Test_all,
                              columns=['MAE_train', 'MAPE_train', 'MSE_train', 'RMSE_train', 'R2_train', 'MAE_test',
                                       'MAPE_test', 'MSE_test', 'RMSE_test', 'R2_test'])

    results_df.to_csv(f'data_evaluate\\xg_{path_list[j]}_evaluate.csv', index=False)

    my_list.append(f"{path_list[j]} 最佳R²值: {best_r2} 对应的随机种子是: {best_seed}")

with open("xg.txt", "w", encoding="utf-8") as file:
    for item in my_list:
        file.write(item + "\n")
